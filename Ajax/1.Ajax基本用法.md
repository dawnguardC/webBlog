...
readyState属性存有 XMLHttpRequest 的状态信息，每当 readyState 改变时，就会触发 readystatechange 事件。

**进度事件**
`progress`，`load`，`error`，`abort`

progress 事件同时存在于下载（xhr对象上）和上传（xhr.upload对象上）的传输，事件对象e的属性保存着数据的的总量（e.loaded）和当前量（e.total）。
