loader
===================
loader 用于对模块的源代码进行解析转换，需要安装。
一个loader在解析完毕后，会返回解析的结果给到下一个loader。
需要配置对象，来说明一种模块要如何进行解析。这个配置对象放在 module.rules 数组内，这个对象包括：
1. test： 匹配方式（正则）
2. use： 要使用的所有loader
...
```
{
    ...
    generator: {
        // 生成图片的配置
        filename: 'imgs/[name]_[hash:8][ext]'// name: 使用图片原名，ext: 使用拓展名，hash：webpack生成的hash，避免图片重名
    }
},
```
webpack5+已经内置了资源的处理，只要说明是资源类型，不需要再下载loader。
