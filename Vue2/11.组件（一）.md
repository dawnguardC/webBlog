...
###  组件非prop的 Attribute
组件元素的attr属性除了prop以外，默认都会传递到组件的根元素上。这其中，class和style会和根元素的叠加，其他的会覆盖到根元素上。
...
`$attrs`是组件元素非prop的attr组成的对象，根元素内的其他元素可以使用`v-bind="$attrs"`获取组件所有非prop的attr:
```
<template id="myComp">
    <div>
        <input v-bind="$attrs">
...
`$listeners`是组件元素所有事件监听器组成的对象，使用`v-on="$listeners"`能将事件监听器指向组件的某个子元素：
```
...
```
value对于text和radio作用是不同的，text可以直接使用，raido则需要将value和自身的value值对比来确定checked。所以对于radio，checkbox等，需要添加model，覆盖默认的 :value 和 @input：
```
<!-- 子组件模版 -->
model: {
  prop: 'parevalue',
...
```
这样就指定了组件v-model的值selected放在parevalue这个传入子组件，并且监听change自定义事件，当事件触发传递一个新的值的时候，更新selected的值。
...
**vue2.6+及vue3中则使用v-slot:name代替slot='name'**，`v-slot:name`可以缩写为`#name`，如果name是变量的话，就使用中括号括起来`#[name]`。
...
### 组件事件.native修饰符
.native修饰符会强制将组件的事件转为「原生 DOM 事件」，并直接绑定到组件的根元素上。
```
...
### .sync 修饰符
.sync是@update:propName的语法糖。
```
<!-- 父组件中 -->

<Child :title.sync="titleValue" />
<!-- 等价于 -->
<Child :title="titleValue" @update:title="titleValue = $event" />
```
...
v-model+model和.sync在实现单个字段双向绑定的最终效果上是等价的，多字段绑定场景下，.sync 是唯一选择。
Vue 3 统一了两者的逻辑，移除了 .sync 修饰符和 model 选项，统一用 v-model:字段名 实现。