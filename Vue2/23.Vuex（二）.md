...
#### namespaced
默认情况下模块是没有开启命名空间的，所有的 state、mutations、actions、getters 都会注册到全局中。组件的commit和dispatch会将根和module的同名函数都触发，可以传入第三个参数`{root: true}`表明是调用的根的函数。

模块添加namespaced属性，就可以隔离开。
```
const moduleA = {
  // 开启命名空间
  namespaced: true,
...
// 访问命名空间模块的state
let name = computed(() => store.state.moduleA.name);

// 访问命名空间模块的getters
let getName = computed(() => store.getters['moduleA/getName']);

// 调用命名空间模块的mutations
let updateName = () => {
  store.commit('moduleA/mutationName', 'wxc');
};

// 调用命名空间模块的actions
let updateNameAsync = () => {
  store.dispatch('moduleA/actionNameAsync', 'wxc');
};
```
