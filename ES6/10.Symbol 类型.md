Symbol 类型
===================
对象的属性名是字符串，就存在重复产生覆盖的风险。为此，ES6 引入了一种新的原始数据类型Symbol，表示独一无二的值来作为属性名。Symbol 值通过Symbol函数生成。
...
Symbol()函数的参数只是表示对当前 Symbol 值的描述，相同参数的Symbol函数的返回值是不相等的。
...
Symbol.for()  为 Symbol 值登记的名字，是全局环境的，不管是函数内调用Symbol.for()，还是iframe窗口调用，都是全局环境的同一个值。
