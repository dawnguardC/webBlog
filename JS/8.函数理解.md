...
**解析**
函数在定义后，在调用执行前先会进行解析，解析时，函数会储存自身所在的上下文环境。
...
函数能够访问作用域内的变量，它和这些变量（上下文环境）组合形成了闭包。也就是说，函数在每次创建时生成闭包，“记住”了函数诞生的环境，函数能访问到这个环境内的变量。
如果需要得到函数内的局部变量，正常情况下，这是办不到的。但是，由于函数能“记住”诞生的环境。如果在函数的内部，再定义一个内部函数，而这个内部函数访问了局部变量，则每次调用内部函数，就可以访问到局部变量，这个局部变量会一直存在于内存中。
