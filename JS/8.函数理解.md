...
**this值**
this是函数体内部自动生成的指针，只有在函数被调用时，才动态指向函数的调用者，即谁调用函数，this就指向谁。
函数调用一般是以下四种环境：
1. 直接调用函数
...
在每次创建时，函数“记住”了诞生的环境（作用域），函数访问这个环境内的变量时，函数和它的作用域组合成了闭包。
...
闭包可以使函数内的变量，就算外层函数执行完毕，也不会被销毁，像全局变量一样，还保存在内存中。

闭包是防抖（debounce）、节流（throttle）的核心实现逻辑 —— 保留定时器 ID、上次执行时间等状态：
```
// 防抖：频繁触发时，仅最后一次触发生效
...
console.log(utils.formatDate(Date.now())); // 正常调用
console.log(utils.formatTime); // undefined（私有方法不可访问）
```
