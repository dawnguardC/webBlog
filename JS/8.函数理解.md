...
logNum.bind(obj,1,2)() //123,1,2
```
bind返回的是一个新的函数，调用它才会被执行。所以一般用于像setTimeout、事件绑定时，在函数名后调用。
bind()函数与call()函数接收参数的方式是一样的。
...
在每次创建时，函数“记住”了诞生的环境（作用域），函数能访问到这个环境内的变量。JavaScript 允许函数嵌套，并且内部函数可以访问定义在外部函数中的所有变量和函数，以及外部函数能访问的所有变量和函数。当内部函数在外部函数作用域被访问时，一个闭包就产生了。
